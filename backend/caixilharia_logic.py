...
def _line(raw):
    ...
    return {
        ...
        "sentido_abertura": raw.get("sentido_abertura", "") or "",
...
def caixilharia_summary(raw, products, families):
    ...
    for line in lines:
        ...
        line_summaries.append({
            ...
            "altura_mm": line["altura_mm"], "sentido_abertura": line["sentido_abertura"],
            "opcoes": options,
        })
...
def caixilharia_email_lines(
    raw, products, families, materials=None, locks=None, handles=None, shutters=None, warning="",
):
    ...
    for line_index, line in enumerate(spec["linhas"], 1):
        ...
        row = (f"{line_index}. {product}{name} — {line['quantidade']} un — "
               f"{line['largura_mm']} x {line['altura_mm']} mm (L x A)")
        if line.get("sentido_abertura"):
            row += f" — Abertura: {line['sentido_abertura']}"
        lines.append(row)
