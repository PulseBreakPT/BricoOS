...
export default function AttachmentManager({ ownerKind, ownerId }) {
  ...
  return (
    <div className="space-y-2">
      {loading ? (
        <div className="flex justify-center py-3"><Spinner className="h-4 w-4 text-slate-400" /></div>
      ) : files.length === 0 ? (
        <p className="text-xs text-slate-400">Sem anexos.</p>
      ) : (
        <div className="space-y-1.5">
          {files.map((f) => (
            <div key={f.id} data-testid={`attachment-${f.id}`} className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-2">
              <FileText className="h-3.5 w-3.5 shrink-0 text-slate-400" />
              <a href={withDeviceToken(`${API}/attachments/${f.id}`)} target="_blank" rel="noreferrer" className="min-w-0 flex-1 truncate text-xs font-semibold text-slate-700 hover:underline">
                {f.filename}
              </a>
              <button type="button" data-testid={`attachment-remove-${f.id}`} onClick={() => remove(f.id)} className="shrink-0 rounded p-1 text-slate-300 hover:text-red-500">
