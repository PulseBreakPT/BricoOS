  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="attachment-preview" className="max-w-4xl border-0 bg-transparent p-0 shadow-none">
        <div className="flex max-h-[85vh] flex-col overflow-hidden rounded-2xl bg-slate-950">
          <div className="flex items-center justify-between gap-2 border-b border-white/10 p-3">
            <p className="min-w-0 truncate text-sm font-semibold text-white">{attachment.filename}</p>
            <a href={attachment.url} download={attachment.filename} target="_blank" rel="noreferrer" className="shrink-0">
              <Button size="sm" variant="outline" className="rounded-xl bg-white/10 text-white hover:bg-white/20" data-testid="attachment-download">
